...
from agent.capture import CaptureThread
...
class AgentUI(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        metrics_card = self._make_metrics_card()
        sidebar.addWidget(metrics_card)
        sidebar.addStretch()
        ...
        header.addWidget(self.metric_packets)
        header.addWidget(self.metric_alerts)
        header.addWidget(self.metric_pps)
        header.addWidget(self.metric_cloud)
        header.addStretch()
        ...
        self.log_table = QtWidgets.QTableWidget(0, 2)
        ...
        # Cloud status ping and tray notifications
    ...
    def add_log(self, msg):
        ...
        self.log_table.setItem(row, 0, QtWidgets.QTableWidgetItem(ts))
        self.log_table.setItem(row, 1, QtWidgets.QTableWidgetItem(msg))
        self.log_table.scrollToBottom()
        # Also reflect in live feed as info
        self._append_feed(ts, "N/A", msg, "info")
    ...
    def _ping_cloud(self):
        ...
        try:
            ...
            ok = False
            latency = 0
        self.metric_cloud._value_lbl.setText(f"{'OK' if ok else 'Down'}")
        if self._last_cloud_ok is None:
            self._last_cloud_ok = ok
            return
        if ok != self._last_cloud_ok:
            if self.tray:
                self.tray.showMessage("Cloud Status", "Cloud reachable" if ok else "Cloud unreachable", QtWidgets.QSystemTrayIcon.MessageIcon.Information)
        self._last_cloud_ok = ok
