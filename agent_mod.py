"""
QuantumDefender Agent — polished modular launcher
Non-destructive: leaves original agent.py untouched.
"""
from __future__ import annotations
import sys, os, socket, uuid, platform, importlib, time, requests
...
def _load_config():
    for modpath in ("agent.config", "config"):
        try:
            ...
            return module.load_config  # type: ignore
        except Exception:
            continue
...
# Import CaptureThread in a way that works when run as script (no package context)
try:
    from capture import CaptureThread  # type: ignore
except ImportError:
    # Add workspace root to path and retry
...
class AgentUI(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("QuantumDefender Agent — Modular")
        ...
        # Sidebar: identity + status + metrics
        sidebar = QtWidgets.QVBoxLayout()
        sidebar.setSpacing(12)
        sidebar.addWidget(self._make_card("Agent Identity", [
    ...
    def _ping_cloud(self):
        ...
        if getattr(self, "_last_cloud_ok", None) is None:
            ...
            return
        if ok != self._last_cloud_ok and hasattr(self, "tray") and self.tray:
